...
//	timestamp: 2013-01-23 17:07:35.627000
//==================================================
...
typedef struct SHAL_basicIOList {
	const THAL_basicIO const *		pL;		// a pointer to a constant array filled with the type informations
...
typedef struct SHAL_VariableList {
	const THAL_Variable const *		pL;		// a reference to the array of the variables
...
extern TAPMMU AP_MMU_create (int elementsNumber);
...
extern TAPMMUmemmory AP_MMU_get (TAPMMU mmu, int elements);
...
typedef struct SHALFunctionParamList {
	const THALFunctionParam const * 	pL;			// the list
	const int							number;		// number of list elements
} THALFunctionParamList;

// the HAL function definition
typedef struct SHALFunction {
	const int						fid;		// function id
	const THALFunctionParamList		paramList; 	// a list with the parameters
	const TpfktHALfunction			pfktHAL;	// the hall function pointer
} THALFunction;
...
extern const THALFunctionList gHALFunctionList;
